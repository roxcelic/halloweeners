    ...
    public override void action(pauseMenuController PMC, string input = "") {
        ...
        switch (eevee.conf.autoDetect()) {
            case eevee.inputCL.controller:
                foreach (string buttonName in eevee.inject.retrieve().FullConfig[key].CONTROLLER_name) PMC.log(buttonName, "controller input", "white");

                break;
            case eevee.inputCL.keyboard:
                foreach (int keyCode in eevee.inject.retrieve().FullConfig[key].KEYBOARD_code) PMC.log(((KeyCode)keyCode).ToString(), "keyboard input", "white");
